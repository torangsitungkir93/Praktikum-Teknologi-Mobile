        ...
        switch (MATCHER.match(uri)) {
            case CODE_MENU_DIR:
                ...
                AppDatabase.initDatabase(context).movieDAO()
                        .insertMovie(MovieTable.fromContentValues(values));
    ...
    public int bulkInsert(@NonNull Uri uri, @NonNull ContentValues[] valuesArray) {
        switch (MATCHER.match(uri)) {
            case CODE_MENU_DIR:
                ...
                final AppDatabase database = AppDatabase.initDatabase(context);
                final MovieTable[] movie = new MovieTable[valuesArray.length];
                for (int i = 0; i < valuesArray.length; i++) {
                    movie[i] = MovieTable.fromContentValues(valuesArray[i]);
                }
